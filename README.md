# Калькулятор для мастеркласса SQA-days 2025

Простое веб-приложение-калькулятор с REST API для демонстрации различных фич взаимодействия с кодом на мастерклассе по AI агентам.

## Описание

Этот проект представляет собой веб-калькулятор с двумя основными компонентами:

### Основная функциональность
- **REST API**: полный набор endpoints для математических операций
- **Базовые математические операции**: сложение, вычитание, умножение, деление
- **Обработка ошибок**: корректная обработка деления на ноль и некорректных данных
- **Покрытие тестами**: все API endpoints покрыты тестами

### Дополнительная фича
- **Округление чисел**: операция round с **максимально неоднозначной логикой** (множественные ветки!)
- **История вычислений**: сохранение всех выполненных операций в JSON файл
- **Персистентность**: история сохраняется между запусками приложения
- **Управление историей**: просмотр и очистка истории через API

## Структура проекта

```
├── calculator.py      # Основная логика калькулятора
├── app.py            # Flask веб-приложение с REST API
├── test_calculator.py # Unit-тесты для основной функциональности
├── test_api.py       # Тесты для API endpoints
├── main.py           # CLI интерфейс (legacy)
├── requirements.txt  # Зависимости проекта
└── README.md        # Этот файл
```

## Установка и запуск

### Установка

1. Убедитесь, что у вас установлен uv. Если нет, [скачайте и установите](https://docs.astral.sh/uv/getting-started/installation/) его

    Проверка, что `uv` установлен:
   ```bash
   uv --version
   ```

2. Установите Python и зависимости проекта через `uv`:
   ```bash
   uv python install
   uv pip install -r requirements.txt
   ```

### Запуск веб-приложения

Запустите приложение с помощью `uv`:
```bash
uv run python app.py
```

Приложение будет доступно по адресу: http://localhost:5000

### Запуск CLI интерфейса

Для тестирования через командную строку:
```bash
uv run python main.py
```

CLI поддерживает все операции включая неоднозначную логику процентов.

### Запуск тестов
```bash
uv run pytest
```

## API Endpoints

### Проверка работоспособности
- `GET /api/health` - проверка статуса API

### Математические операции
- `POST /api/add` - сложение
- `POST /api/subtract` - вычитание  
- `POST /api/multiply` - умножение
- `POST /api/divide` - деление
- `POST /api/round` - округление чисел (максимально неоднозначная логика!)
- `POST /api/calculate` - универсальный endpoint

### Работа с историей
- `GET /api/history` - получить историю вычислений
- `DELETE /api/history` - очистить историю

### Примеры запросов

#### Сложение
```bash
curl -X POST http://localhost:5000/api/add \
  -H "Content-Type: application/json" \
  -d '{"a": 2, "b": 3}'
```

#### Получение истории
```bash
curl http://localhost:5000/api/history
```

#### Округление чисел (максимально неоднозначная логика!)
```bash
# Округление до знаков после запятой
curl -X POST http://localhost:5000/api/round \
  -H "Content-Type: application/json" \
  -d '{"value": 3.14159, "precision": 2, "method": "auto"}'

# Округление до разрядов
curl -X POST http://localhost:5000/api/round \
  -H "Content-Type: application/json" \
  -d '{"value": 1234, "precision": -2, "method": "up"}'

# Разные методы округления
curl -X POST http://localhost:5000/api/round \
  -H "Content-Type: application/json" \
  -d '{"value": 2.5, "precision": 0, "method": "banker"}'
```

#### Универсальный endpoint
```bash
curl -X POST http://localhost:5000/api/calculate \
  -H "Content-Type: application/json" \
  -d '{"operation": "multiply", "a": 4, "b": 5}'
```

#### Универсальный endpoint для степени
```bash
curl -X POST http://localhost:5000/api/calculate \
  -H "Content-Type: application/json" \
  -d '{"operation": "power", "base": 3, "exponent": 4}'
```

## ⚠️ Неоднозначная логика операции округления

Операция `round` имеет **максимально неоднозначную логику** с множественными ветками:

### Правила интерпретации:
- **Если `precision > 0`**: округление до precision знаков после запятой
- **Если `precision < 0`**: округление до precision разрядов (10^precision)
- **Если `precision == 0`**: округление до целого
- **Если `precision дробное`**: округление до ближайшего целого precision
- **Методы округления**: auto, up, down, banker, truncate
- **Особые случаи**: value == 0, очень маленькие числа, отрицательные значения

### Примеры неоднозначности:
```bash
# Одинаковые precision, разные методы:
round(2.5, 0, "up") = 3.0      # округление вверх
round(2.5, 0, "down") = 2.0   # округление вниз
round(2.5, 0, "banker") = 2.0 # банковское округление

# Разные precision, одинаковый результат:
round(3.14159, 2, "auto") = 3.14    # до 2 знаков
round(314, -2, "auto") = 300        # до 2 разрядов

# Особые случаи:
round(0, 2, "auto") = 0.0           # ноль
round(1e-15, 2, "auto") = 0.0       # очень маленькое число
```

### Тестирование через CLI:
```bash
# Запуск CLI интерфейса
uv run python main.py

# Выберите пункт 5 (Округление числа)
# Попробуйте разные значения:
# - value: 3.14159, precision: 2, method: auto
# - value: 3.14159, precision: 2, method: up
# - value: 1234, precision: -2, method: auto
# - value: 0, precision: 2, method: auto
```

### Задачи для исследования:
1. **Исследовать все методы округления и их различия**
2. **Найти все граничные случаи для операции округления**
3. **Понять логику автовыбора метода округления**
4. **Написать тесты для всех сценариев**
5. **Предложить улучшения API**

## Особенности для мастеркласса

1. **Простота**: код легко читается и понимается
2. **Тестируемость**: основная функциональность и API покрыты тестами
3. **Расширяемость**: легко добавить новые операции или фичи
4. **Понятная тематика**: калькулятор знаком всем участникам
5. **REST API**: современный подход к разработке веб-приложений
6. **Дополнительная фича**: история вычислений - понятная без объяснений

## Возможные задачи для мастеркласса

### Расширение функциональности
- Добавление новых математических операций (корень, логарифм, тригонометрические функции)
- Добавление валидации входных данных
- Реализация аутентификации и авторизации
- Добавление rate limiting для API
- **Исследование и тестирование неоднозначной логики округления**

### Исследование неоднозначной логики
- Анализ поведения операции round с различными входными данными
- Написание comprehensive тестов для всех edge cases
- Документирование всех сценариев поведения
- Предложение улучшений API для устранения неоднозначности
- Рефакторинг логики для большей ясности

### Улучшение API
- Добавление версионирования API
- Реализация пагинации для истории
- Добавление фильтрации и сортировки истории
- Создание OpenAPI/Swagger документации

### Тестирование и качество
- Написание интеграционных тестов
- Добавление тестов производительности
- Реализация CI/CD pipeline
- Добавление мониторинга и логирования

### Архитектура и рефакторинг
- Разделение на микросервисы
- Добавление базы данных вместо файлового хранения
- Реализация кэширования
- Добавление Docker контейнеризации
